...
import {
  ...
  MOUSE_DOWN,
  TARGETBOX_DRAG,
  TARGETBOX_DRAG_START,
  TARGETBOX_DRAG_END,
  TARGETBOX_RESIZE,
  TARGETBOX_RESIZE_START,
  TARGETBOX_RESIZE_END,
  CONTEXT_MENU,
  MAKE_GROUP,
  UNMAKE_GROUP,
  COPY_BOX,
...
const makeGroup = (state, action) => {
  ...
  for (const boxId of state.get("selectedBoxIds").toArray()) {
    tempStyle.top = Math.min(
      state.getIn([ "boxData", "boxList", boxId, "top", ]),
      tempStyle.top
    );
    tempStyle.left = Math.min(
      state.getIn([ "boxData", "boxList", boxId, "left" ,]),
      tempStyle.left
    );
    tempStyle.right = Math.max(
      state.getIn([ "boxData", "boxList", boxId, "left" ,]) +
        state.getIn([ "boxData", "boxList", boxId, "width" ,]),
      tempStyle.right
    );
    tempStyle.bottom = Math.max(
      state.getIn([ "boxData", "boxList", boxId, "top" ,]) +
        state.getIn([ "boxData", "boxList", boxId, "height" ,]),
      tempStyle.bottom
    );
  }
  ...
  const newBox = Map({
    top: tempStyle.top,
    left: tempStyle.left,
    width: tempStyle.right - tempStyle.left,
    height: tempStyle.bottom - tempStyle.top,
    background: "transparent",
    childBoxIds: state.get("selectedBoxIds"),
  });

  const newBox_boxHierarchy = Map({
    boxIds: state
      .get("selectedBoxIds")
      .reduce(
        (map, key) =>
          map.concat(state.getIn([ "boxData", "boxHierarchy", key, "boxIds" ,])),
        state.get("selectedBoxIds")
      ),
    boxHierarchy: state
      .get("selectedBoxIds")
      .reduce(
        (map, key) =>
          map.set(key, state.getIn([ "boxData", "boxHierarchy", key ,])),
        Map({})
      ),
  });
  let newBoxHierarchy = state
    .get("selectedBoxIds")
    .reduce(
      (map, key) => map.delete(key),
      state.getIn([ "boxData", "boxHierarchy" ,])
    );
  newBoxHierarchy = newBoxHierarchy.set(
    String(state.get("idCount")),
    newBox_boxHierarchy
  );
  ...
  return state.withMutations(map =>
    map
      ...
      .updateIn([ "boxData", "boxIds" ,], boxIds =>
        boxIds.push(String(state.get("idCount")))
      )
...
const unmakeGroup = (state, action) => {
  ...
  const boxId = state.getIn([ "selectedBoxIds", 0 ,]);
  if (
    getHierarchy(state.getIn([ "boxData", "boxHierarchy", ]), boxId).get("boxIds")
      .size === 0
  )
    return state;
  ...
  return state.withMutations(map =>
    map
      ...
      .set("contextMenu", contextMenuInitialState)
      .updateIn([ "boxData", "boxIds" ,], boxIds =>
        boxIds.filter(value => value !== boxId)
      )
      .deleteIn([ "boxData", "boxList", boxId ,])
      .updateIn([ "boxData", "boxHierarchy" ,], boxHierarchy =>
        boxHierarchy.concat(
          state.getIn([ "boxData", "boxHierarchy", boxId, "boxHierarchy" ,])
        )
      )
...
const targetBoxDragStart = (state, action) =>
  state.withMutations(map =>
    map
      ...
      .updateIn([ "boxData", "boxList", ], boxList =>
        boxList.map((value, key) => {
          if (state.get("selectedBoxIds").includes(key)) {
            return value
              ...
              .set(
                "orgWidth",
                state.getIn([ "boxData", "boxList", key, "width" ,])
              )
              .set(
                "orgHeight",
                state.getIn([ "boxData", "boxList", key, "height" ,])
              );
          }
          return value;
        })
...
const targetBoxDrag = (state, action) => {
  ...
  const dragAreaRect = document
    .getElementById("dragArea")
    .getBoundingClientRect();
  action.x = Math.min(
    Math.max(action.x, dragAreaRect.left),
    dragAreaRect.left + dragAreaRect.width
  );
  action.y = Math.min(
    Math.max(action.y, dragAreaRect.top),
    dragAreaRect.top + dragAreaRect.height
  );

  const targetBox = state.get("targetBox").toObject();
  const dragAmount = {
    left: action.x - targetBox.x,
    top: action.y - targetBox.y,
  };
  const topDiff = targetBox.realTop - targetBox.top;
  const leftDiff = targetBox.realLeft - targetBox.left;

  const allSelectedBoxIds = getChildBoxIds(
    state.getIn([ "boxData", "boxHierarchy" ,]),
    state.get("selectedBoxIds")
  );
  ...
  let newState = state.withMutations(map =>
    map
      ...
      .updateIn(
        [ "targetBox", "realLeft", ],
        realLeft => realLeft + dragAmount.left
      )
      .updateIn(
        [ "targetBox", "top" ,],
        top => top + dragAmount.top + topDiff + topDiff2
      )
      .updateIn(
        [ "targetBox", "left", ],
        left => left + dragAmount.left + leftDiff + leftDiff2
      )
      .setIn([ "targetBox", "x" ,], action.x)
      .setIn([ "targetBox", "y", ], action.y)
  ...
  for (const boxId of allSelectedBoxIds) {
    newState = newState.withMutations(map =>
      map
        .updateIn(
          [ "boxData", "boxList", boxId, "top", ],
          top => top + dragAmount.top + topDiff + topDiff2
        )
        .updateIn(
          [ "boxData", "boxList", boxId, "left", ],
          left => left + dragAmount.left + leftDiff + leftDiff2
        )
    );
  }
...
const targetBoxDragEnd = (state, action) =>
  state.set("snapLine", snapLineInitialState);

const targetBoxResizeStart = (state, action) => {
  const ret = getContainerRect(
    state
      .getIn([ "boxData", "boxList" ,])
      .filter((value, key) => state.get("selectedBoxIds").includes(key))
  );
...
const targetBoxResize = (state, action) => {
  ...
  newTargetBox = newTargetBox.withMutations(map =>
    map.set("x", action.x).set("y", action.y)
  );
  ...
  for (const boxId of allSelectedBoxIds) {
    newState = newState.withMutations(map =>
      map
        ...
        .updateIn(
          [ "boxData", "boxList", boxId, "height" ,],
          height => (newTargetBox.height / targetBox.height) * height
        )
        .updateIn(
          [ "boxData", "boxList", boxId, "width", ],
...
const deleteBox = (state, action) => {
  ...
  const newBoxIds = getChildBoxIds(
    state.getIn([ "boxData", "boxHierarchy" ,]),
    state.get("selectedBoxIds")
  );
  let newBoxHierarchy = state.getIn([ "boxData", "boxHierarchy", ]);

  const deleteBoxHierarchy = (boxHierarchy, id) => {
    ...
    boxHierarchy
      ...
      .forEach((value, key) => {
        newBoxHierarchy = newBoxHierarchy
          .updateIn([ key, "boxIds" ,], boxIds =>
            boxIds.filter((value, key) => value !== id)
          )
          .updateIn([ key, "boxHierarchy" ,], boxHierarchy =>
            deleteBoxHierarchy(boxHierarchy, id)
          );
      });

    return newBoxHierarchy;
  };
  newBoxHierarchy = newBoxIds.reduce(
    (map, key) => deleteBoxHierarchy(map, key),
    state.getIn([ "boxData", "boxHierarchy" ,])
  );
  ...
  return state.withMutations(map =>
    map
      ...
      .set("contextMenu", contextMenuInitialState)
      .updateIn([ "boxData", "boxIds", ], boxIds =>
        boxIds.filter(value => !newBoxIds.includes(value))
      )
      .updateIn([ "boxData", "boxList" ,], boxList =>
        boxList.filter((value, id) => !newBoxIds.includes(id))
      )
      .setIn([ "boxData", "boxHierarchy", ], newBoxHierarchy)
  );
};
const copyBox = (state, action) => {
  ...
  return state.withMutations(map =>
    map
      ...
      .setIn(
        [ "clipBoard", "boxHierarchy" ,],
        state
          .getIn([ "boxData", "boxHierarchy", ])
          .filter((value, key) => state.get("selectedBoxIds").includes(key))
      )
      .setIn(
        [ "clipBoard", "boxList", ],
...
const pasteBox = (state, action) => {
  ...
  const changeBoxIdHierarchy = (boxHierarchy, id, newId) => {
    if (boxHierarchy.size === 0) return boxHierarchy;
    if (boxHierarchy.get(id) !== undefined) {
    ...
    boxHierarchy
      ...
      .forEach((value, key) => {
        newBoxHierarchy = newBoxHierarchy
          ...
          .updateIn([ key, "boxHierarchy" ,], boxHierarchy =>
            changeBoxIdHierarchy(boxHierarchy, id, newId)
          );
      });
  ...
  let newState = state;
  state.getIn([ "clipBoard", "boxIds" ,]).forEach((id, key) => {
    const newId = idCount++;
    newState = newState
      .updateIn([ "clipBoard", "boxIds", ], boxIds =>
        boxIds.map(boxId => {
          if (boxId === id) return String(newId);
          return String(boxId);
        })
      )
      .updateIn([ "clipBoard", "boxList" ,], boxList =>
        boxList.mapKeys(boxId => {
          if (boxId === id) return String(newId);
          return String(boxId);
        })
      );
    const newBoxHierarchy = changeBoxIdHierarchy(
      newState.getIn([ "clipBoard", "boxHierarchy" ,]),
      id,
      newId
    );
    newState = newState.setIn([ "clipBoard", "boxHierarchy" ,], newBoxHierarchy);
  });

  newState = newState.updateIn([ "clipBoard", "boxList", ], boxList =>
    boxList.map((value, key) =>
      value.withMutations(map =>
        map
          .update(
            "top",
            top =>
              top +
              state.getIn([ "contextMenu", "style", "top" ,]) -
              state.getIn([ "clipBoard", "top", ])
          )
          .update(
            "left",
            left =>
              left +
              state.getIn([ "contextMenu", "style", "left" ,]) -
              state.getIn([ "clipBoard", "left", ])
          )
      )
    )
  );
  ...
  return state.withMutations(map =>
    map
      ...
      .set("contextMenu", contextMenuInitialState)
      .updateIn([ "boxData", "boxIds" ,], boxIds =>
        boxIds.concat(newState.getIn([ "clipBoard", "boxIds", ]))
      )
      .updateIn([ "boxData", "boxHierarchy" ,], boxHierarchy =>
        boxHierarchy.merge(newState.getIn([ "clipBoard", "boxHierarchy", ]))
      )
      .updateIn([ "boxData", "boxList" ,], boxList =>
        boxList.merge(newState.getIn([ "clipBoard", "boxList" ,]))
      )
      .set("idCount", idCount)
  );
};
...
const sourceDragEnd = (state, action) => {
  ...
  if (
    action.x < layout.left ||
    action.x > layout.screenWidth - layout.right ||
    action.y < layout.top ||
    action.y > layout.screenHeight - layout.bottom
  ) {
    return state;
  }

  const newBox = state
    .getIn([ "boxSourceList", action.key ,])
  ...
  return state.withMutations(map =>
    map
      ...
      .setIn(
        [ "boxData", "boxHierarchy", String(state.get("idCount")) ,],
        boxHierarchyInitialState
      )
      .update("idCount", idCount => idCount + 1)
  );
...
const mouseDown = (state, action) => {
  ...
  const newSelectedBoxIds =
    action.shift === true
      ? state.get("selectedBoxIds").push(action.id)
      : List([ action.id, ]);
  const selectedBoxList = state
    .getIn([ "boxData", "boxList" ,])
...
const contextMenu = (state, action) => {
  ...
  if (state.get("selectedBoxIds").size === 1) {
    const id = state.getIn([ "selectedBoxIds", 0, ]);
    if (
      getHierarchy(state.getIn([ "boxData", "boxHierarchy" ,]), id).get("boxIds")
        .size !== 0
    ) {
...
const changeProp = (state, action) => {
  ...
  let newState = state.setIn(
    [ "boxData", "boxList", action.boxId, action.propName ,],
    action.propValue
  );
  ...
  const allSelectedBoxIds = getChildBoxIds(
    newState.getIn([ "boxData", "boxHierarchy" ,]),
    newState.get("selectedBoxIds")
  );
  for (const boxId of allSelectedBoxIds) {
