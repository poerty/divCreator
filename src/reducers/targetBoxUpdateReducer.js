...
const drag = action => state => {
  ...
  return state.withMutations(map =>
    map.update("realTop", realTop => realTop + dragAmount.top)
...
const setRealValue = action => state => {
  return state
...
const addBox = action => state => {
  ...
  return state.withMutations(map => {
    ...
    const ret = boxsToBox(_boxs.filter((val, key) => map.get('ids').includes(key)))
    map
      .set("top", ret.top)
      .set("left", ret.left)
      .set("width", ret.width)
